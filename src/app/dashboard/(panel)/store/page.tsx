    ...
    <>
      <header className="dash-header">
        <div className="dash-toolbar">
          <h1 className="text-2xl font-bold text-(--text-primary)">{__('Store Products')}</h1>
      ...
      <main className="dash-main"><div className="dash-inner">
        ...
        <div className="card mt-4 overflow-hidden">
          {products.isLoading ? (
            ...
            <div className="empty-state py-16 text-center">
              ...
              <h3 className="empty-state-title mt-3 text-lg font-semibold text-(--text-primary)">{__('No products')}</h3>
              <p className="empty-state-text mt-1 text-sm text-(--text-muted)">
                {search ? __('No products match your search.') : __('Create your first product to get started.')}
              </p>
              {!search && (
                <Link href={adminPanel.storeProductNew} className="btn btn-primary mt-4 inline-flex">
                  <Plus className="h-4 w-4" />
                  {__('Add Product')}
                </Link>
              )}
                ...
                {(data?.results ?? []).map((p) => (
                  <tr key={p.id} className="table-tr hover:bg-(--surface-secondary)">
                    ...
                    <td className="table-td table-td-primary">
                      ...
                      {p.sku && <p className="text-xs text-(--text-muted)">{p.sku}</p>}
                    </td>
                    <td className="table-td">
                      <span className="text-xs text-(--text-secondary) capitalize">{p.type}</span>
                    </td>
                    <td className="table-td text-sm">{formatPrice(p.priceCents, p.currency)}</td>
                    <td className="table-td">
                      <span className={cn(
                        'badge',
                        p.status === 'published' && 'badge-published',
                        p.status === 'draft' && 'badge-draft',
                        p.status === 'archived' && 'bg-(--surface-secondary) text-(--text-muted)',
                      )}>
        ...
        <ConfirmDialog
          ...
          variant="danger"
          onConfirm={() => { if (deleteTarget) deleteProduct.mutate({ id: deleteTarget.id }); }}
