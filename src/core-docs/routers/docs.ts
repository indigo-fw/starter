...
const localeInput = z.string().max(10).default(DEFAULT_LOCALE).refine(
...
export const docsRouter = createTRPCRouter({
  ...
  getBySlug: publicProcedure
    ...
    .query(async ({ input }) => {
      const doc = await getDocBySlug(input.slug, input.locale);
      if (!doc) {
  ...
  getNavigation: publicProcedure
    ...
    .query(async ({ input }) => {
      return getDocsNavigation(input.locale);
    }),
  ...
  search: publicProcedure
    ...
    .query(async ({ input }) => {
      return searchDocs(input.query, input.limit, input.locale);
    }),

  /** Get LLM-friendly plain text export */
  llmExport: publicProcedure
    .input(z.object({ locale: localeInput }))
    .query(async ({ input }) => {
      return { content: await generateLlmExport(input.locale) };
    }),
  ...
  adminList: docsAdminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const conditions = [];
      if (input.locale) conditions.push(eq(cmsDocs.locale, input.locale));
      ...
      const [items, [countRow]] = await Promise.all([
        ctx.db
          ...
          .from(cmsDocs)
          .where(where)
          .orderBy(asc(cmsDocs.locale), asc(cmsDocs.section), asc(cmsDocs.sortOrder))
          .offset(offset)
          .limit(pageSize),
