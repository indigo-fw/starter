...
Modules are pulled in as git subtrees with `git subtree … --squash`, which doesn't require any shared history — so `indigo add` / `indigo update` work the same whether your project came from `bunx degit` or a `git clone`. (Pushing changes back to a module's upstream repo — `indigo push` — is for module maintainers and only works from the framework repo.)
