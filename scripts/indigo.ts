...
async function remove(id: string, flags: { yes?: boolean; dropTables?: boolean }) {
  ...
  // Collect schema info and extract table names BEFORE deleting anything
  const tableNames: string[] = [];
  if (flags.dropTables) {
    const schemaFiles = await getModuleSchemaFiles(id);
  ...
  // Step 5: Generate DROP migration if requested (table names already extracted above)
  if (flags.dropTables && tableNames.length > 0) {
